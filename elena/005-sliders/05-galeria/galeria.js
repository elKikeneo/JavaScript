...
function listarFotos(cat){
    ...
    for(var i=0; i<listadFotos.length; i++){
        if(listadFotos[i].cat == cat || cat=="todas"){
            sectionFotos.innerHTML += "<div onclick='verFoto("+listadFotos[i].id+")' class='foto' style='background:url("+listadFotos[i].src+") no-repeat center; background-size:cover'></div>";
...
function verFoto(idFoto){
    ...
    for(var i=0; i<listadFotos.length; i++){
        if(listadFotos[i].id == idFoto){
            contenedorFoto.innerHTML = "<div class='fotoView'><img src='"+listadFotos[i].src+"'><p>"+listadFotos[i].titulo+"</p></div>";
        }
    }
